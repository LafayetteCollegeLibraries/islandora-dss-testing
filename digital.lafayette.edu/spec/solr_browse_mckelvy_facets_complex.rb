...
describe "SolrBrowseMdlPrintsFacetSubjectLcsh" do
  ...
  describe 'browsing search results by any facet within 2 Solr fields for the McKelvy House Photograph Collection' do
    ...
    [
     ...
     'Subject',
      ...
      describe 'browsing search results by the initial 5 Solr facets for the field #{field_label} within the McKelvy House Photograph Collection' do

        (1..5).each do |facet_index|
  
          it "refines the search by the facet #{facet_index} for faceted field #{field_label}" do

            @driver.get(@base_url + "/collections/mckelvy")
