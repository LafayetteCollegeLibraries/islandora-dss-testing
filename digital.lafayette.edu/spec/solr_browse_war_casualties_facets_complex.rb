...
describe "SolrBrowseMdlPrintsFacetSubjectLcsh" do
  ...
  describe 'browsing search results by any facet within 4 Solr fields for the World War II Casualties collection' do
    ...
    [
     ...
     'Description.Military.Rank',
      ...
      describe 'browsing search results by the initial 5 Solr facets for the field #{field_label} within the World War II Casualties collection' do

        (1..5).each do |facet_index|
  
          it "refines the search by the facet #{facet_index} for faceted field #{field_label}" do

            @driver.get(@base_url + "/collections/war")
