...
describe "SolrBrowseMdlPrintsFacetSubjectLcsh" do
  ...
  describe 'searching by the initial 4 Solr facets for the Marquis de Lafayette Prints Collection' do
    ...
    (1..4).each do |i|
      ...
      (1..5).each do |j|
        ...
        it "test_solr_browse_mdl_prints_facet_subject_lcsh" do
          ...
          subject_lcsh_elem = @driver.find_element(:xpath, "//ul[@class='islandora-solr-facet-list'][#{i}]/li[#{j}]/a")

          # subject_lcsh_bucket_count = @driver.find_element(:xpath, "//ul[@id='mdl-prints-subject-lcsh']/li[#{i}]/span").text
          subject_lcsh_bucket_count = @driver.find_element(:xpath, "//ul[@class='islandora-solr-facet-list'][#{i}]/li[#{j}]/span").text
